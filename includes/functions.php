...
function get_posts($user_id = 0, $sort = false){
    ...
    if($sort) $sorting = 'ASC';
    if($user_id>0){
        return db_query("SELECT posts.*, users.name, users.login, users.avatar FROM `posts` JOIN `users` ON users.id = posts.user_id WHERE posts.user_id = $user_id ORDER BY `posts`.`date` $sorting")->fetchAll();
    }
...
function add_user($login, $pass){
    ...
    return db_query("INSERT INTO `users` (`id`, `login`, `pass`, `name`) VALUES 
    (NULL, '$login', '$password ', '$name')", true);
}

function register_user($auth_data){
    if(empty($auth_data) || !isset($auth_data['login']) || empty($auth_data['login']) || !isset($auth_data['pass']) || empty($auth_data['pass']) || !isset($auth_data['pass2']) || empty($auth_data['pass2'])){
        return false;
    }
    ...
    if(strlen($auth_data['pass'])<6){
        $_SESSION['error'] = 'Пароль не может быть меньше 6 символов';
        header("Location: " . get_url('register.php'));
        die;
    }

    if(strlen($auth_data['pass'])>32){
        $_SESSION['error'] = 'Пароль не может быть больше 32 символов';
        header("Location: " . get_url('register.php'));
        die;
    }

    if(strlen($auth_data['login'])>32){
        $_SESSION['error'] = 'Логин не может быть больше 32 символов';
        header("Location: " . get_url('register.php'));
        die;
    }
